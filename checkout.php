...
if (isset($_POST['order'])) {
    ...
    if ($check_cart->rowCount() > 0) {
        $insert_order = $conn->prepare(
            'INSERT INTO `orders`(user_id, name, number, email, method, address, total_products, total_price) VALUES(?,?,?,?,?,?,?,?)'
        );
   ...
   <h3>your orders</h3>
      ...
      if ($select_cart->rowCount() > 0) {
          while ($fetch_cart = $select_cart->fetch(PDO::FETCH_ASSOC)) {
              ...
              ?>
         <p> <?= $fetch_cart['name'] ?> <span>(<?= $fetch_cart['price'] .
      'DT'.
     ' x ' .
     $fetch_cart['quantity'] ?>)</span> </p>
      ...
      <input type="submit" name="order" class="btn <?= $grand_total > 1
          ? ''
          : 'disabled' ?>" value="place order">
